...
template<typename T>
__global__ void convert_data(const T* data1, const T* data2, double* fdata1, double* fdata2, int size) {
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < size; idx += gridDim.x * blockDim.x) {
...
template<typename T>
__global__ void gen_data(curandState_t* state, T* data, double* ground_truth, int myRank, int nRanks, int size) {
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < size; idx += gridDim.x * blockDim.x) {
        double sum = 0.0;
        for (int i = 0; i < nRanks; i++) {
...
template<typename T>
void run(int myRank, int nRanks, ncclComm_t& comm, int threads, int block_limit, int data_size, bool performance_test) {
    T*           result;
    cudaStream_t stream;
    ...
    CUDACHECK(hipExtMallocWithFlags(
        (void**)&buffer, 2 * data_size * sizeof(T) + sizeof(vllm::Signal), hipDeviceMallocUncached));
#else
    ...
    MPICHECK(MPI_Allgather(&self_data_handle,
    ...
    for (int i = 0; i < nRanks; i++) {
        if (i == myRank)
            ...
            CUDACHECK(cudaIpcOpenMemHandle((void**)&ipc_ptrs[i], data_handles[i], cudaIpcMemLazyEnablePeerAccess));
    }
    vllm::CustomAllreduce fa(ipc_ptrs, rank_data, rank_data_sz, myRank, nRanks);
    auto*                 self_data =
        reinterpret_cast<T*>(reinterpret_cast<char*>(buffer) + sizeof(vllm::Signal) + data_size * sizeof(T));
    // hack buffer registration
    {
        void* data[8];
        for (int i = 0; i < nRanks; i++) {
            data[i] = ((char*)ipc_ptrs[i]) + sizeof(vllm::Signal) + data_size * sizeof(T);
        }
    ...
    CUDACHECK(cudaMemcpyAsync(self_data_copy, self_data, data_size * sizeof(T), cudaMemcpyDeviceToDevice, stream));
    ...
    if (performance_test) {
        ...
        if (myRank == 0)
            printf("Rank %d done, nGPUs:%d, sz (kb): %d, %d, %d, my time:%.2fus, nccl "
                   ...
                   myRank,
                   nRanks,
                   data_size * sizeof(T) / 1024,
                   threads,
                   block_limit,
                   duration_ms * 1e3 / num_iters,
        ...
        if (myRank == 0)
            std::cout << "average abs diffs: nccl: " << nccl_diffs / data_size << " me: " << my_diffs / data_size
                      << std::endl;
    } else {
        for (int i = 0; i < 100; i++) {
            fa.allreduce<T>(stream, self_data, result, data_size, threads, block_limit);
            ...
            convert_data<T><<<108, 1024, 0, stream>>>(self_data_copy, result, nccl_result, my_result, data_size);
            CUDACHECK(cudaStreamSynchronize(stream));
