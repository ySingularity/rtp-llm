...
MMRdmaEncoderOp::MMRdmaEncoderOp(const VitConfig& vit_config) {
    transport_ = createMMRdmaTransport(vit_config, MMRdmaRole::ENCODER_SERVER);
}
...
void registerMMRdmaEncoderOp(py::module& m) {
    py::class_<MMRdmaEncoderOp, std::shared_ptr<MMRdmaEncoderOp>>(m, "MMRdmaEncoderOp")
        .def(py::init<const VitConfig&>(), py::arg("vit_config"))
