...
def init_vit_group_args(parser, vit_config):
    ...
    vit_group.add_argument(
        "--mm_rdma_qp_count_per_connection",
        env_name="MM_RDMA_QP_COUNT_PER_CONNECTION",
        bind_to=(vit_config, "mm_rdma_qp_count_per_connection"),
        type=int,
        default=2,
        help="每条 RDMA 连接的 QP 数",
    )
    vit_group.add_argument(
        "--mm_rdma_io_thread_count",
        env_name="MM_RDMA_IO_THREAD_COUNT",
        bind_to=(vit_config, "mm_rdma_io_thread_count"),
        type=int,
        default=2,
        help="RDMA io 线程数",
    )
    vit_group.add_argument(
        "--mm_rdma_worker_thread_count",
        env_name="MM_RDMA_WORKER_THREAD_COUNT",
        bind_to=(vit_config, "mm_rdma_worker_thread_count"),
        type=int,
        default=2,
        help="RDMA worker 线程数",
    )
    vit_group.add_argument(
