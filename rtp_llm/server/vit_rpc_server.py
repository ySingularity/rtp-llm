...
from rtp_llm.ops import VitConfig as CppVitConfig
...
def _build_cpp_vit_config(vit_config):
    """Map the relevant mm-rdma fields from the Python VitConfig onto a C++ VitConfig
    so it can be handed to MMRdmaEncoderOp."""
    cpp = CppVitConfig()
    cpp.mm_rdma_enable = bool(getattr(vit_config, "mm_rdma_enable", False))
    cpp.mm_rdma_port = int(getattr(vit_config, "mm_rdma_port", 0))
    cpp.mm_rdma_connect_timeout_ms = int(getattr(vit_config, "mm_rdma_connect_timeout_ms", 250))
    cpp.mm_rdma_qp_count_per_connection = int(
        getattr(vit_config, "mm_rdma_qp_count_per_connection", 2)
    )
    cpp.mm_rdma_io_thread_count = int(getattr(vit_config, "mm_rdma_io_thread_count", 2))
    cpp.mm_rdma_worker_thread_count = int(getattr(vit_config, "mm_rdma_worker_thread_count", 2))
    cpp.mm_rdma_read_timeout_ms = int(getattr(vit_config, "mm_rdma_read_timeout_ms", 30 * 1000))
    cpp.mm_rdma_slot_gc_timeout_ms = int(getattr(vit_config, "mm_rdma_slot_gc_timeout_ms", 60 * 1000))
    return cpp
...
class MultimodalRpcServer(MultimodalRpcServiceServicer):
    def __init__(self, mm_process_engine: MMProcessEngine, vit_config=None):
        ...
        if vit_config is not None and getattr(vit_config, "mm_rdma_enable", False):
            try:
                rdma = MMRdmaEncoderOp(_build_cpp_vit_config(vit_config))
                if rdma.enabled():
                    self._rdma = rdma
