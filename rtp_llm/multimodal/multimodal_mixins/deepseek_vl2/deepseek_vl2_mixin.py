...
class DeepSeekVLV2Mixin(BaseMultiModalMixin):
    ...
    def _get_mm_module(cls, mm_related_params: VitParameters, vit_config: VitConfig):
        # vit_config is unused for DeepSeek-VL2 (image normalization lives in
        # mm_related_params.config["vision_config"]); previously it was passed
        # positionally as `image_mean`, silently corrupting the ImageTransform.
        # Match runtime _init_multimodal which instantiates with defaults.
        mm_part = DeepSeekVLV2ImageEmbedding(mm_related_params)
